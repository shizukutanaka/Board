...
# ⚠️ CI ワークフローは存在しない (v1.7.79 の要件監査で判明)。
# 「v1.7.58 で作成済み・未コミット」と長く書かれていたが、`.github/` はリポジトリにも
# ローカルにも無い — 過去セッションの使い捨てコンテナと共に消えている。
# CI が守る不変条件は全て `node test.mjs` が既に実行する。自動化は5段階アルゴリズムの
# 最終段なので、要件の削除・単純化を終えてから着手する (docs/feature-backlog.md 末尾)。
```
