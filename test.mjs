...
const checks = [
  ...
  ['hidden shapes leave search + bindAt', html.includes("_sv(s)&&_lc((_lb(s)||'')+(s.text||'')+(s.type||'')")&&html.includes("t!=='pen'&&_sv(s)")],
  ...
  ['dash applies to frame+image borders in canvas+SVG', html.includes("_D6.has(s.type)||_conn(s.type)")&&html.includes('fill="none" stroke="${_esc(_sk(s))}" stroke-width="${_num(s.size)}"${dA}')],
  ...
  ['image caption band honours s.fill (canvas+SVG)', html.includes("c.fillStyle=_fi(s)&&_fi(s)!=='none'?_fi(s):_p()||'#fff';c.globalAlpha=0.85")&&html.includes("_esc(_fi(s)||paper||'#FFFFFF')")],
