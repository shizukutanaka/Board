...
残る欠落機能（コネクタ束縛・マルチページ・回転・レイヤーパネル・シェイプ検索）は
いずれも **大コスト**。実装はヒットテスト / bbox / SVG・PNG 出力 / sync op / undo の
広範囲に波及し、最小実装でも 1.5KB+ gzip を要する。現在の予算余裕は ~660B のみ。

CLAUDE.md の原則「一気に全部は作らない・各 Phase は別 ADR + 独立リリース」に従い、
これらは個別の予算回収パス + 専用 ADR を伴う独立リリースとして扱う:

- **コネクタ束縛** (P2, 高): arrow 端点をシェイプに束縛し移動追従。`drawArrow` /
  `G.hit(line)` / `G.bbox(line)` / SVG・minimap 出力 / `endLineLike` / エッジ交点計算に波及。
- **回転** (P3, 中): 全シェイプ型の回転ヒットテスト・bbox・ハンドル・描画が必要で最大級。
- **マルチページ / レイヤーパネル / 検索** (P3): UI 面積と state モデル拡張が大きい。

次に着手するなら最高価値の **コネクタ束縛**。専用 ADR で設計（束縛の保存形式・
派生端点を描画時に計算する方式・undo/sync 整合）を固めてから実装する。
