...
- **同期の運用性**: WebRTC は手動シグナリング(URL 手渡し)。シグナリングサーバ無しは長所だが
  「URL を開くだけで共同編集」には届かない。プレゼンス(他者カーソル)は ADR-0010 で実装済み。
  他者の**選択状態**のハイライトは同 ADR で意図的にスコープ外(将来の別 ADR)。**[P1]**
...
| 優先 | 項目 | 概要 | 形態 |
|---|---|---|---|
| P2 | プレゼンス(選択ハイライト) | 他者の選択状態のブロードキャスト(カーソルは ADR-0010 で実装済み) | ADR + 独立リリース |
