...
| P3 | 画像参照分離 段階2 | ワイヤーレベル (op/共有リンク/RTC) で `img` 参照化 — 段階1 (IDB blob 分離) は ADR-0031 で済み | ADR |
