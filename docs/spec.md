...
### ✅ v1.7.72 で解消(First Principles 監査 — 勝利条件「小さく保つ」の公称値乖離)
- **サイズ公称値の 38% 過少申告**: README バッジ/競合比較表・CLAUDE.md・本仕様の計5箇所が
  `~61KB gzip`(本仕様は `~56KB`)としていたが実測は gzip 85KB / raw 268KB。2026-06-13 の
  gzip 予算撤去時に検証機構ごと失われたのが根因(`test.mjs` は gzip を計算していたが
  assert せず `console.log` のみ)。実測値へ訂正し、**README バッジ ±10% の整合テスト**を追加
  して再発を構造的に封じた(v1.6.77 の version バッジ ↔ `const V` 拘束と同手法)。
- **raw サイズを文字数で計測**: `html.length` は UTF-8 文字数で、多バイト文字により実バイト数を
...
- **共有リンクの暗号化に関する誇大表記**: README 比較表が「E2E 暗号化 … URL fragment key」と
  記載する一方、実装に `crypto.subtle`/AES-GCM は皆無で共有 URL は deflate+base64 の平文
  だった(第三者が鍵なしで全文復元可能なことを実測で確証)。表記を「✗ (未実装 — 計画中)」に
  訂正し、Share モーダルとREADMEに非暗号化の警告を追加。本質的な暗号化実装は FT-21 として
  backlog 化(要 ADR: 鍵配置・後方互換・`file://` でのセキュアコンテキスト制約)。
...
- ~~**DOM ミラー a11y**~~ **→ v1.7.74 (ADR-0016) で実装**。canvas の外に画面外の
  `role="region"` を常時保持し、盤面の要約(型別の件数)と z 順の全図形一覧を browse mode /
  ローターから**一度に**読めるようにした。canvas 自体にも `aria-describedby` で要約を接続。
  タブストップは増やさず `aria-live` にもしない(編集のたびの全文読み上げを避ける)。
  200 図形の上限を超えた分は「ほか N 個」と明示し、要約は常に盤面全体を数える。
  **残る未充足**: 実機スクリーンリーダーでの読み上げ確認(FT-11)、および空間的関係
  (重なり・グループの入れ子・frame の親子)は一覧に反映していない点。
...
### 14.2 短所(既知の弱み)
- **a11y の天井**: ~~図形ごとの DOM ミラーが無く、スクリーンリーダーは個々の図形を辿れない~~
  → v1.7.74 (ADR-0016) で**フラットな一覧ミラー**を実装。残る天井は **木構造**である:
  一覧は z 順の平坦な列で、グループの入れ子・frame の親子・重なりの空間関係は表現しない。
  実機 SR での検証も未実施(FT-11)。**[P3 に降格]**
...
- ~~**大規模スケール**~~ **→ 要件を削除 (v1.7.79、実測による)**。「>2000 図形で線形処理が重い」は
  **一度も計測されていない主張**だった。実測: **10,000 図形(主張の閾値の5倍)で走査 1.21ms**、
  ヒットテスト 0.194ms。しかも**盤面サイズに比例しない** — viewport カリングが既に
  「画面に写っているもの」でコストを縛っているため。quadtree / dirty-rect は
  **存在すべきでない要件**だったので、作らずに消した。`test.mjs` の perf ガードが
  この判断を守る(超線形化したら落ちる = 要件が証拠付きで復活する)。
...
| 優先 | 項目 | 概要 | 形態 |
|---|---|---|---|
| ~~P1~~ ✅ | DOM ミラー a11y | 画面外 DOM に盤面の要約 + z 順一覧 — **v1.7.74 / ADR-0016 で実装** | 完了 |
| P3 | 階層ミラー | 上記の一覧を group / frame の入れ子として木構造化 | ADR |
...
| ~~P3~~ ❌ | 空間索引(quadtree) | **削除** — 実測で不要と判明 (v1.7.79)。カリングが既にコストを画面サイズで縛る | 作らない |
