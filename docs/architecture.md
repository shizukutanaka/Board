...
## 回転の既知の制約 (v1.6.62 — ソクラテス問答監査)

`shape.rotate` (度) は描画 (`drawShape` の `ctx.rotate`)・ヒットテスト (`G.hit` の逆回転)・
bbox 包絡矩形 (`G.bbox`)・SVG/PNG 出力・反転 (`flipShape` で角度を反転) に対応。
複数選択回転 (`doRotate`) は選択 bbox 中心で各シェイプを公転させる (`doFlip` と同じ群中心
セマンティクス)。単一選択はその場回転。`,`/`.` キーで ±15°、ロックシェイプは除外。
