...
## ハーネスが被試験体に親切をすると、その分だけ検査は嘘になる (v1.7.86 / ADR-0019)

ADR-0018 の教訓「ソースを見る検査は結果を見る検査に負ける」と対になる、もう一つの失敗様式。

`test.mjs` の2ピア収束ハーネスは、冒頭でこう書いている:

```js
A.state.peerId='peerA'; B.state.peerId='peerB';
```

テストを書きやすくするための、ごく自然な一行である。しかし実際には
**製品が保証していなかった性質を、ハーネスが親切心で供給していた**。

実ブラウザでは両タブが `localStorage['board.peer']` から**同一の id** を読む。
CRDT の重複排除キー `clock.peer + ':' + clock.seq` はレプリカ単位の一意な id を前提と
するため、2タブの N 番目の op は区別不能になる。そして `Store.commit()` は `_apply` の
**前に** `seenOps` を検査する (index.html:1256) — 衝突した側は送信に失敗するのではなく、
**自分の編集を自分で捨てる**。ユーザーが図形を描いても、何も現れない。

さらに `test.mjs` には、その **バグを不変条件として固定する存在検査**まであった:

```js
['PEER_ID persistence', html.includes("localStorage.getItem('board.peer')")],
```

正しく直すと**この検査が落ちる**。存在検査が誤った設計を保護した3例目。

### 一般化できる規則

1. **ハーネスがセットアップで代入する値は、すべて「検証されていない仮定」である。**
   `peerId` を手で振り分けた瞬間、「id はレプリカ単位で一意」という命題は
   *テストされる対象* から *テストの前提* に移動した。前提は決して落ちない。
2. **セットアップの一行は、仕様の一行かもしれない。** ハーネスが与えている値を列挙し、
   「これは製品が本当に保証しているか?」を1つずつ問う。保証しているなら**それを検査に
   する**。していないなら**バグである**。
3. **同一性 (identity) は、規模を跨ぐと壊れる。** 「ブラウザごとに一意」は
   「レプリカごとに一意」を意味しない。分散システムで id を持つときは、
   **どの粒度で一意なのかを名前に書く**(本件はコメントが `persistent per browser` と
   正直に書いており、そこに答えが書いてあった — 誰も問わなかっただけ)。
4. **症状ではなく同一性を疑う。** `seq` を単調にする / キーに `ts` を混ぜる、はいずれも
   衝突「確率」を下げるだけで同一性の欠陥を残す。壊れているのは `peer` の側だった。

対応する結果レベルの検査は `sync-browser.mjs` (実 Chromium の2タブ、12検査、
修正前ビルドでは12中4が落ちる)。「主張が実態を上回る」**7例目**。
