...
## 3.18 ソクラテス式の新視点 — 「過不足の機能」(2026-07-01, ADR-0004 直後)

§3.9 で identity(scratchpad/workspace)の未決定を指摘したが、その決定を待たずに検証できる
問いが残っている: **現行の機能一覧のうち、実際に「過剰」(コストが価値を上回る)なものはあるか、
逆に「不足」(§3.9 の決定に依存せず今すぐ埋められるべき穴)はあるか?** 4つの仮説を立て、
コードを読んで反証を試みた(§3.5「否定は反証可能でなければ信用できない」に従う)。

- **問: フォーマットペインター(Alt+C/V)は過剰か?** → **反証**。`copyStyle`/`pasteStyle` は
  計20行程度で、スタイルパネルと共用の `applyStyleToSelection` を再利用するのみ。独立実装コストは
  ほぼゼロ。「使わない人にコストを強制しない」機能は過剰の基準を満たさない。
- **問: 「PDFエクスポート」は実装と名前が食い違う誇大表示か?** → **半ば反証**。`exportPDF()`
  (index.html:3335) は canvas をラスタライズし `window.print()` を呼ぶだけで、Board 自身は
  1バイトも PDF を生成しない。しかし UI 上の表示は `⌘P` ショートカットと help grid の `'PDF'`
  という一語のみで、「独自 PDF エクスポート機能」を偽ってはいない。「印刷 → OS の PDF 保存」は
  単一HTML・依存ゼロ制約下でのブラウザアプリの標準的な慣習であり、外部 PDF ライブラリを積む方が
  むしろ原則違反。実装として妥当。
- **問: WebRTC 手動シグナリングに自動再接続が無いのは実装漏れか?** → **半ば反証**。切断は
  `dc.onclose`(index.html:3734-3735)で検知され `disconnected` トーストが出る(無音の失敗では
  ない)。だが「サーバーを介さない」設計そのものが自動再接続の土台(リレー/シグナリングサーバー)を
  排除しており、これは実装漏れではなく **正体(§3.8 capability-based, no server)の必然的帰結**。
- **問: ミニマップは小規模ボードでも常時表示する価値があるか?** → **実証(唯一生き残った仮説)**。
  `.minimap-wrap{opacity:.85}`(index.html:311-320)で常時表示、非表示になるのはプレゼンモード中
  のみ(`_savedVp` と同様に `Presentation.enter/leave` が `display:none` を出し入れする箇所のみ)。
  図形が数個しかないボードでも右下に常駐し、ユーザーが閉じる手段が一切ない。「使わない人にゼロコスト」
  という良い機能の条件(1点目の判定基準)に**違反している**。

**新視点**: 4仮説中3つが反証されたこと自体が発見である。Board の機能一覧は見た目ほど「機能過多」
ではなく、大半は「使わなければゼロコスト」という条件を満たしている。真に軽い実装コストで直せる
不足はミニマップのトグルのみで、他の「不足に見えるもの」(構造化エクスポート・複数バージョン保存・
プレゼンス)はすべて §3.9 の identity 決定に従属するか、原則(no-server)の必然的帰結であり、
機能追加ではなく **製品判断** の領域に属する。

**実装(この節の直後にコミット)**: ミニマップに非表示切り替え(トグル)を追加。
