...
**この視点からの falsifiable な処置(本コミットで着手)**:
- 二者ハーネスに **衝突の characterization テスト**を追加。同一プロパティへの同時編集が **現状
  *発散する*ことを assert** する(`A.stroke !== B.stroke`)。これは「バグを正とする」テストではなく、
  **既知の限界を反証可能に固定する**もの。version/versionNonce LWW(研究 item K)が入った瞬間に
  この assert は**壊れ**、実装者に「ここを収束に書き換えよ」と強制する forcing function になる。
- §3.14 の緑が「sync 完成」と誤読されるのを、隣に置いた赤い真実(発散の事実)で打ち消す。

> 帰結: 収束は **証明されていない**。互いに素な編集でのみ成立し、衝突では発散する。これは
> 欠陥の告白ではなく **保証範囲の正確な確定**だ。「Board の同期は *衝突しない協働* で動く」——
> ここまでが真。「衝突しても安全」は item K(LWW)を ADR 付きで入れるまで、約束してはならない。
