...
- Verdict(途中経過): `FIX`(ただし高リスク、実装は慎重に) — **部分緩和済み** (v1.7.77,
  ...
  0.009% を確認。dirty-rect 本体(drawGrid/選択/オーバーレイ込みの部分再描画)は
  依然 L 級・未着手。
...
## FT-15 — 画像 dataURL の参照分離(state 肥大化の緩和) — 段階1 実装済み (v1.7.89, ADR-0031)
- Verdict: `PARTIAL`(2026-09-23)。段階1(永続化層)は ADR-0031 で実装済み: IDB v2 の
  `imgs` オブジェクトストアに content-hash キーで blob 分離し、doc レコードは
  `img` 参照のみ保持。`:prev` バックアップが blob を共有し孤児は save 時 GC。
  段階2(op・共有リンク・RTC blob 交換をワイヤーレベルでも参照化)は未着手 —
  ワイヤ上では依然 dataURL がインラインで流れる。
- Verdict(当初): `FIX`
