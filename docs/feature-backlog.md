...
- Evidence: `docs/a11y-audit-2026-07.md`「残作業」。README のアクセシビリティ節には
  ...
  破綻なく操作できるかは未検証。
- **2026-08-25 (v1.7.83): 要件を問い直して大半を機械検証に移した。** 「実機 SR が要る」は
  1年間そのまま据え置かれていたが、**一度も疑われていなかった**。疑うとこうなる:

  > NVDA も VoiceOver も**読み上げ内容を発明しない**。プラットフォームのアクセシビリティ
  > ツリー(Windows: IAccessible2 / UIA、macOS: NSAccessibility)を読む。そのツリーは
  > ブラウザが DOM + ARIA から accname / role アルゴリズムで計算したもので、Chromium は
  > **それ自体を** CDP `Accessibility.getFullAXTree` で外に出す。

  つまり「人間でなければ分からないこと」は当初思われていたよりずっと小さい:

  | | 誰が確認するか |
  |---|---|
  | 名前が**在るか** / 名前が**正しいか** / role / state / ミラーがツリーに**到達しているか** | **`a11y-browser.mjs`(機械・CI)** |
  | 読み上げが**快いか** — 冗長さ・実際の読み順・音にしたときの自然さ | 人間 (縮小した残作業) |

  `a11y-browser.mjs` を追加(依存ゼロ。Node 22 の内蔵 WebSocket で CDP を話す。
  Playwright も puppeteer も使わない)。実 Chromium 141 に対し28項目を検証:
  45個のコントロール全てがブラウザ計算のアクセシブル名を持つこと、記号だけの名前が
  無いこと(NVDA はコードポイントを読み上げてしまう)、ADR-0016 のミラーが
  `clip-path:inset(50%)` で隠れてもツリーから**刈られていない**こと、canvas が
  無名の穴でないこと、`file://` の capability 3点(ADR-0017 限界4を解決)、
  そして⌘A→⇧F10→↓→Enter の**マウス皆無**の整列完遂。
- **初回実行で実バグを1件発見** — 詳細は下記「発見」。**これが「source を見る検査」と
  「result を見る検査」の差**で、test.mjs には原理的に見えないクラスだった。
- 発見: `#ctx` の keydown は `window` まで**バブルする**ため、メニュー用の
  `_ctxMenuKeyNav` と window のショートカット配送器が**両方**走っていた。結果、
  メニューを矢印で辿るだけで選択図形が1押下につき1pxずつ動く。マウス利用者は
  絶対に踏まない経路で、ハンドラを直接叩く test.mjs にも見えない
  (二重配送は**実 DOM にしか存在しない**)。v1.7.83 で isolation ガードを追加。
- Action(残り): 実機 SR での**読み上げ品質**の確認。名前・role・state の正しさは
  もう機械が見ており、CI に入れば**退行しない**。
- Effort: S(縮小済み。かつては全部が人間側だった)
- Depends on: none
...
### FT-15 — 完了 (v1.7.84)。**実ブラウザで測ってから残りを決めた**

v1.7.80 の上表は `test.mjs` の合成データによる推定だった。v1.7.83 で実ブラウザを
駆動できるようになったので、**実 Chromium で本物の JPEG を使って測り直した**:

| 盤面 | 保存 (IndexedDB) | 共有リンク |
|---|---|---|
| 1200×900 JPEG (dataURL 1.16MB) | **20ms、往復で生存** | **1,167,158 文字** / 974ms (上限の71倍) |
| 2400×1800 (4.6MB) | — | 取込ゲート (4MB) で拒否 |

**これで要件が二つに割れた**:
- **保存側は壊れていない。作らない** — 20ms で保存でき、リロード後も画像は残る。
  要件の半分を**実測で削除**。
- **共有側の実害は「待ち時間」ではなく「成功したように見えるダミー」**だった。
  警告は v1.7.80 で出るようになっていたが、**Copy ボタンは生きたまま**で、
  ユーザーはコピーし、送り、相手側で失敗する。上限超過時は URL を出さず Copy を無効化。

**書いた直後に削除した最適化 — 記録として残す**: 画像 dataURL 長を合計して巨大な盤面では
約1秒の deflate+AES を丸ごと省く近道を実装した。論拠は「画像 dataURL は圧縮済みバイト列
なので deflate は縮められない ⇒ **偽陽性のない下界**」。**論拠は誤りで、テストが捕まえた** —
合計するだけでは中身が高エントロピーである保証が無く、圧縮可能なペイロード
(非圧縮 BMP、合成 dataURL) では 21,406 バイトの「下界」が 429 文字の URL になる。
**共有できる盤面を「できません」と断るところだった**。動くはずのものを断るのは1秒待たせる
より悪いので、誤差係数で誤魔化さず削除。反例は `test.mjs` に回帰テストとして残っている。
判定は**実際の長さ**で行う — 推定しない。
...
`node coverage.mjs` が指した未実行経路を、大きい順に上から潰した結果:

| サブシステム | 結果 |
|---|---|
| 共有リンク (`Share`) | **実バグ2件** — 平文だった / deflate が一度も実行されず URL が7.2倍 |
| WebRTC (`_wrtcInit`) | **実バグ1件** — 接続失敗が完全に無反応 |
| 起動時復元 (`Persist.load`) | **実バグ1件** — 読めない図形を黙って捨て、次の保存で上書き |
| キーダウン配送 | 健全(3つのガードを固定) |
| ポインタ層 | 健全(ただしテスト初稿が3回フィクスチャ誤りで通っていた) |
| `.board` 往復 | 健全(group/lock/rotate/束縛/ラベル/z順/非ASCII すべて保存) |
| コンテキストメニュー | 健全(全項目にハンドラと role) |
| ホイール/ドロップ | 健全(ズーム不変量が 1e-9 で成立、4MB ゲート機能) |
| 描画(全8型) | 健全(全型が描画呼び出しを発行、文字も canvas に到達) |
| fit / clear-all | 健全(空盤面で NaN にならない、拒否は完全な no-op) |

**10 サブシステム中 4 件に実バグ**。未実行関数 155 → **131**。
残りは大半が (a) 既に覆われた関数の内部クロージャ(V8 の遅延コンパイル)、
(b) ブラウザ専用(`exportPDF` は印刷ウィンドウを開く)。

**「テストが緑」と「経路が実行されている」は別物**、というのがこの作業の要約。
1600件超が緑のまま、上記4件は何年も生き残っていた。
