...
「ADR-0019 の修正は本当に**全部**直したのか?」と問い直した回。
バグは出なかったが、**未検証だった柱が1本と、自分が持ち込んだコストの記載漏れが1件**見つかった。

### Added
- **WebRTC の「成功パス」を実ブラウザで初めて検証** (`sync-browser.mjs`、12→18検査)。
  FT-20 は「繋がらないリンク」のフィードバックを追加し、`test.mjs` は失敗側の配線を
  固定していたが、**実際に繋がって op が渡ることは一度も観測されていなかった**
  (fake ハーネスは `RTCPeerConnection` ごとスタブする)。手動シグナリング
  (offer → answer → consume) を2タブ間で実行し、`connectionState==='connected'`、
  DataChannel が両側で `open`、`rtc:` 合成ピア id での追跡までを見る。
  **транспорт の分離**: 同一ルームでは BroadcastChannel が op を運んでしまい主張が空虚に
  なるため、接続後に**両側の BroadcastChannel を閉じてから**描画する。
  `Net.init(room)` による部屋分けは**製品 UI から到達不能**(`main()` は引数なしで呼ぶ)なので、
  ユーザーが取れない経路に依存させるより確実。
  **非空虚性**: `wrtcAcceptOffer` を無効化すると **18中5が落ちる**ことを確認済。

### Fixed
- **ADR-0019 に、自分の修正が持ち込んだコストの記載が漏れていた**。
  `peerId` を per-load にしたことで、**リロードのたびに「幽霊ピア」が最大15秒残る**
  (修正前は id を使い回すため `lastSeen` が更新されるだけだった)。
  実測して ADR の「既知の限界」に追記: `NET_PRESENCE_TIMEOUT` (15秒) 以内に
  `_reapPeers` が回収し、影響はピア数バッジの一時的な過大表示のみで自己回復する。
  **`bye` メッセージで即座に消す案は採らなかった**: クラッシュや強制終了では発火せず、
  どのみち回収器が要る。ハッピーパスだけの2つ目の機構は複雑さを増やすだけ。
  回収器自体は `test.mjs` で既に固定済みのため**検査も追加しない**(同じ不変条件を二度書かない)。

## [1.7.86] - 2026-09-02

**同期の柱に、静かなデータ損失があった。**「主張が実態を上回る」の**7例目** —
今回は文書でも検査でもなく、**ハーネス自身**が実態を上回っていた。

### Fixed
- **【同期】同一ブラウザの2タブが同じ `peerId` を共有し、片方が自分の編集を自分で捨てて
  いた** (ADR-0019)。`PEER_ID` は `localStorage['board.peer']` から読まれ、コメントは
  「persistent per browser」と書いていた。しかし CRDT の重複排除キーは
  `clock.peer + ':' + clock.seq` で、一意性は**レプリカ単位の id** を前提とする。
  **2タブは2つの独立レプリカ**で各々 `seq` を 0 から数えるため、N 番目の op が区別不能に
  なる。そして `Store.commit()` は `_apply` の**前に** `seenOps` を検査する
  (index.html:1256) — 衝突した側は送信に失敗するのではなく、**自分の描画を自分で捨てる**。
  **ユーザーが図形を描いても何も現れない。** 実 Chromium の2タブで実測:
  `IDENTICAL: true` / `A: shapes=1` `B: shapes=2` (恒久的な乖離、エラーなし)。
  単一タブのリロードでも `seq` が 0 に戻るため、WebRTC の相手ピアは
  **リロード後の編集を `seq` が旧最大値を超えるまで全部捨てていた**。
  → **永続化を削除**し `const PEER_ID=uid();` の1行に。4行が1行になり、バグが2つ消えた。
  永続性を必要とする用途は1つも無かった (重複排除は不要、スナップショット選出は順序だけ、
  アバター頭2文字は装飾)。**永続性が一意性を破っていた。**
- **README L108「同一ブラウザのタブ間: BroadcastChannel で即時同期」は片方向しか
  成立していなかった** (通る向きは相手の `seq` が遅れている間だけの偶然)。

### Added
- **`sync-browser.mjs`** (新規, 依存ゼロ, `a11y-browser.mjs` / `offline-browser.mjs` の兄弟):
  **実 Chromium の2タブ**で同期を検証する (計12検査)。2タブが異なるレプリカ id を持つこと、
  IndexedDB の起動時引き継ぎ、**A→B と B→A の双方向**伝播、**ADR-0015 の複製 undo** が
  相手タブに反映されること、**リロード後の編集も届く**こと、最後に**両タブが同一の盤面**を
  持つこと (件数ではなく内容で比較)。**修正前ビルドでは 12 中 4 が落ちる**ことを確認済。
- `offline-browser.mjs` を15検査に拡張: オンライン中に描いた図形が**オフラインのリロードを
  越えて IndexedDB から生還する**ことを追加検証 (「キャッシュされた空の殻」を渡していない
  ことの証明)。
- CI (`docs/ci-workflow.yml`) と `.githooks/pre-push` に上記を追加。

### Changed
- **バグを不変条件として固定していた存在検査を反転**:
  `['PEER_ID persistence', html.includes("localStorage.getItem('board.peer')")]` は
  **バグの実装を要求していた** (正しく直すと落ちる検査)。存在検査が誤った設計を保護した
  3例目。レプリカ単位であること・永続化しないことを主張する検査に置換。
- **修正の理由そのものを固定する振る舞いテストを追加**: 共有 id では B が自分の描画を
  自分で捨てて両盤面から消えること / 異なる id なら同じ2つの commit が両方生き残ること。
  2ピアハーネスが `peerA`/`peerB` を**手で代入**していた事実 —
  **製品が破っていた当の性質をハーネスが供給していた** — もコメントに明記した。
- `docs/architecture.md` に「ハーネスが被試験体に親切をすると、その分だけ検査は嘘になる」
  を記録。`docs/spec.md` に `peerId` の MUST 条項。
...
(「dataURL を document から分離する」)で書かれていて、**実害は一度も測られていなかった**。

### 測定結果 (実 Chromium)

| 盤面 | 保存 | 共有リンク |
|---|---|---|
| 1200×900 JPEG (dataURL 1.16MB) | **20ms、IndexedDB 往復で生存** | **1,167,158 文字** (974ms、上限の71倍) |
| 2400×1800 (4.6MB) | 取込ゲートで拒否 (4MB) | — |

### Fixed
- **貼っても届かないリンクに Copy ボタンが生きていた**。これが実害の正体だった —
  待ち時間ではなく、**成功したように見えるダミーを渡していたこと**。ユーザーはコピーし、
  送り、相手側で失敗する。上限超過時は URL を出さず Copy を無効化し、`.board` 書き出しへ
  誘導する。判定は**実際の長さ**で行う(推定しない)。
- 文言も実態に合わせた: 「長すぎて共有できない**可能性が高い**」→ 大きすぎるので
  **URL は表示していない**、と実際にやったことを述べる (ja/en 両方)。

### Removed
- **書いた直後に削除した最適化**: 画像 dataURL の長さを合計して、巨大な盤面では
  deflate+AES の約1秒を丸ごと省く近道を実装した。論拠は「画像 dataURL は既に圧縮済みの
  バイト列だから deflate は縮められない ⇒ **偽陽性のない下界**」。**この論拠は誤りで、
  テストが捕まえた** — 合計するだけでは中身が高エントロピーである保証がどこにも無い。
  非圧縮 BMP や合成 dataURL のような**圧縮可能な**ペイロードでは、21,406 バイトの「下界」が
  実際には 429 文字の URL になった。つまり**共有できる盤面を「できません」と断る**ところ
  だった。**動くはずのものを断るのは、1秒待たせるより悪い**。誤差係数で誤魔化さず削除。
  この反例はテストとして残してある。
- **保存側の要件は実測で削除**。20ms・往復生存で、壊れていない。作らない。

### Changed
- `test.mjs` の共有モーダル検査を**行の丸ごと固定から性質の記述へ**。
  `getElementById('shareEnc').hidden=!r.encrypted;` を完全一致で固定していたため、
  正当な `||tooLong` を足しただけで落ち、**存在しない不誠実さを報告した**。
  「エクスポート結果から導出され、極性が正しい」ことだけを述べる正規表現に変更。
